...
use rustdb_executor::{eval_with, ExecError, Relation, SubqueryRunner, TableSource};
use rustdb_planner::{bind, plan, Catalog};
use rustdb_sql::statement::{Join, Select, SelectItem, Statement, TableRef};
...
impl CorrelatedRunner {
    ...
    fn run_query(
        ...
        rustdb_executor::run(&physical, &self.source)
    }
}
...
fn substitute_select(
    ...
    Select {
        ...
        order_by: s
            .order_by
            .iter()
            .map(|o| rustdb_sql::statement::OrderItem {
                expr: sub(&o.expr),
                desc: o.desc,
            })
            .collect(),
